...
<script>
	...
	export default {
		...
		computed:{
			obj(){
				return this.$store.getters.getObjs
			}
		},
		components: {
			Modal, Edit
		...
		methods: {
			...
			del(index){
				let patient ={
					index:index,
					id:this.obj[index].id
				}
				this.$store.dispatch('delPatient',patient)
			},
			edit(index){
				this.editObj = this.obj[index]
				this.$store.commit('editModalToggle',true)
