<template>
<div>
	<Modal></Modal>
	<Edit :editObj="editObj"></Edit>
		<div class="patients">
		<div class="title">
			Bemorlar
		</div>
		<div class="table">
			<table>
				<tr>
					<th>Ism-sharifi</th>
					<th>Email</th>
					<th>Sana</th>
					<th>Qabul vaqti</th>
					<th>Shifokor</th>
					<th>Sabab</th>
					<th></th>
					<th></th>
				</tr>
				<tr v-for="(item, index) of obj" :key="index">
					<td>{{item.name}}</td>
					<td>{{item.email}}</td>
					<td>{{item.number}}</td>
					
					<td>{{item.doc}}</td>
					<td>{{item.reason}}</td>
					<td>{{item.date}}
					</td>
					<td>
						<router-link tag="button" :to="'/PatientInfo/'+item.id" class="more"><svg width="19" height="13"
								viewBox="0 0 19 13" fill="none" xmlns="http://www.w3.org/2000/svg">
								<path
									d="M18.788 5.76856C17.7983 4.14989 16.4676 2.84929 14.796 1.86681C13.1243 0.884291 11.359 0.392975 9.5 0.392975C7.64098 0.392975 5.87566 0.884105 4.20396 1.86681C2.53223 2.84929 1.20155 4.14989 0.211975 5.76856C0.0706708 6.01593 0 6.2598 0 6.50019C0 6.74057 0.0706708 6.98445 0.211975 7.23174C1.20155 8.85048 2.53223 10.1512 4.20396 11.1334C5.87566 12.1161 7.64098 12.607 9.5 12.607C11.3591 12.607 13.1243 12.1177 14.796 11.1388C16.4676 10.1598 17.7985 8.85742 18.788 7.23174C18.9294 6.98445 19 6.74068 19 6.50019C19 6.2598 18.9294 6.01593 18.788 5.76856ZM7.22557 2.86868C7.85816 2.2361 8.61623 1.91969 9.49989 1.91969C9.64119 1.91969 9.76144 1.96931 9.86045 2.0682C9.9593 2.16713 10.009 2.28735 10.009 2.42869C10.009 2.57014 9.9593 2.69028 9.86067 2.78906C9.76181 2.8881 9.64149 2.93757 9.50015 2.93757C8.8923 2.93757 8.37261 3.15326 7.94149 3.58434C7.51023 4.01568 7.29472 4.53514 7.29472 5.14299C7.29472 5.28452 7.24511 5.40458 7.14621 5.50359C7.04717 5.60267 6.92707 5.6521 6.78573 5.6521C6.64428 5.6521 6.52414 5.60256 6.42514 5.50359C6.32609 5.40458 6.27663 5.28437 6.27663 5.14299C6.27663 4.25952 6.59303 3.5013 7.22557 2.86868ZM14.1069 9.96212C12.6897 10.8207 11.1541 11.2503 9.5 11.2503C7.84598 11.2503 6.31038 10.821 4.89318 9.96212C3.47598 9.10327 2.2973 7.94933 1.35723 6.50019C2.43162 4.83198 3.77816 3.5843 5.39683 2.75746C4.96567 3.49246 4.75002 4.28774 4.75002 5.14296C4.75002 6.45076 5.21486 7.56905 6.14427 8.49869C7.07369 9.42832 8.19239 9.89294 9.5 9.89294C10.8077 9.89294 11.9261 9.42806 12.8557 8.49869C13.7854 7.56931 14.25 6.45076 14.25 5.14296C14.25 4.28774 14.0343 3.49235 13.6031 2.75746C15.2217 3.5843 16.5683 4.83209 17.6427 6.50019C16.7028 7.94933 15.5242 9.10327 14.1069 9.96212Z"
									fill="#2F80ED" /></svg></router-link>
						<button class="edit" @click="edit(index)"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"
								xmlns="http://www.w3.org/2000/svg">
								<path
									d="M0 15.3663C0.4448 13.8399 0.888 12.3135 1.3328 10.7871C1.368 10.6655 1.4048 10.5439 1.4368 10.4223C1.488 10.2383 1.584 10.0895 1.7312 9.96631C1.8688 9.85271 1.9904 9.71671 2.1168 9.59031C4.2016 7.52471 6.2864 5.45912 8.3696 3.39192C8.4176 3.34392 8.4592 3.28952 8.4928 3.24792C8.776 3.53272 9.0416 3.79992 9.3376 4.09752C9.3056 4.11992 9.2432 4.15192 9.1952 4.19992C7.0352 6.35512 4.8768 8.51031 2.7216 10.6703C2.6448 10.7487 2.5808 10.8559 2.5504 10.9615C2.2912 11.8175 2.0448 12.6783 1.7888 13.5375C1.7568 13.6447 1.7728 13.7119 1.8544 13.7855C1.9872 13.9055 2.104 14.0431 2.2384 14.1615C2.2848 14.2015 2.3728 14.2319 2.4288 14.2175C3.2896 13.9711 4.1488 13.7199 5.0064 13.4607C5.08 13.4383 5.1488 13.3759 5.2064 13.3167C7.3968 11.1327 9.584 8.94551 11.7712 6.75991C11.8128 6.71831 11.848 6.67191 11.872 6.64471C12.1824 6.95511 12.4832 7.25591 12.8128 7.58551C12.7984 7.59511 12.7408 7.61911 12.7008 7.65911C10.544 9.79511 8.384 11.9263 6.2384 14.0735C5.9392 14.3743 5.6144 14.5599 5.2128 14.6735C3.7888 15.0767 2.3712 15.5023 0.9504 15.9199C0.856 15.9471 0.76 15.9727 0.6656 15.9999H0.5376C0.4688 15.9695 0.3984 15.9423 0.3296 15.9103C0.1376 15.8207 0.072 15.6383 0 15.4607V15.3663ZM4.3024 12.7151C4.2608 12.4495 4.216 12.2063 4.1872 11.9631C4.1728 11.8479 4.1264 11.8015 4.0144 11.7807C3.7408 11.7279 3.4704 11.6591 3.2176 11.6031C5.4864 9.32951 7.7552 7.05431 10.0096 4.79352C10.3696 5.15352 10.7456 5.52792 11.1088 5.88952C8.8624 8.14231 6.5952 10.4159 4.3024 12.7151ZM10.8368 1.02553C11.0416 0.825529 11.2464 0.61273 11.4656 0.41433C12.0944 -0.152068 12.9136 -0.139268 13.5264 0.45913C14.2032 1.11833 14.8704 1.78553 15.5296 2.46233C16.1536 3.10072 16.144 3.95512 15.5216 4.59192C15.3344 4.78392 15.1408 4.97112 14.9616 5.14712C13.592 3.77912 12.224 2.41273 10.8368 1.02553ZM9.168 2.53592C9.4784 2.24473 9.7968 1.94553 10.0992 1.66073C11.5328 3.09432 12.9584 4.52152 14.3968 5.95992C14.0976 6.23992 13.776 6.54232 13.4656 6.83351L9.168 2.53592Z"
									fill="#A0A4A8" />
							</svg>
						</button>
						<button class="del" @click="del(index)"><svg width="12" height="14" viewBox="0 0 12 14" fill="none"
								xmlns="http://www.w3.org/2000/svg">
								<path
									d="M2.00957 13.3248C2.02557 13.7024 2.33597 14 2.71357 14H9.29117C9.66877 14 9.98077 13.7024 9.99517 13.3248L10.4656 3.408H1.53917L2.00957 13.3248ZM7.48957 5.872C7.48957 5.7136 7.61757 5.5856 7.77597 5.5856H8.23357C8.39197 5.5856 8.51997 5.7152 8.51997 5.872V11.5376C8.51997 11.696 8.39037 11.824 8.23357 11.824H7.77597C7.61757 11.824 7.48957 11.696 7.48957 11.5376V5.872ZM5.48797 5.872C5.48797 5.7136 5.61597 5.5856 5.77437 5.5856H6.23197C6.39037 5.5856 6.51837 5.7152 6.51837 5.872V11.5376C6.51837 11.696 6.39037 11.824 6.23197 11.824H5.77437C5.61597 11.824 5.48797 11.696 5.48797 11.5376V5.872ZM3.48477 5.872C3.48477 5.7136 3.61277 5.5856 3.76957 5.5856H4.22717C4.38557 5.5856 4.51357 5.7152 4.51357 5.872V11.5376C4.51357 11.696 4.38397 11.824 4.22717 11.824H3.76957C3.61117 11.824 3.48477 11.696 3.48477 11.5376V5.872ZM10.7776 0.7216H7.74397V0.1472C7.74397 0.0656 7.67837 0 7.59677 0H4.40957C4.32797 0 4.26237 0.0656 4.26237 0.1472V0.7216H1.22717C0.983968 0.7216 0.785568 0.9184 0.785568 1.1632V2.552H11.2192V1.1632C11.2192 0.92 11.0224 0.7216 10.7776 0.7216Z"
									fill="#FF6760" />
							</svg></button>
					</td>
				</tr>
			</table>
		</div>
		<div class="next">
			<div class="numbers">
				<button class="one">
					<img src="../assets/img/left.svg" alt="">
				</button>
				<button class="one">
					1
				</button>
				<button class="one">
					2
				</button>
				<button class="one">
					3
				</button>
				<button class="one">
					<img src="../assets/img/right.svg" alt="">
				</button>
			</div>

			
			<div class="man">
				<button class="man" @click="man()">
					<img src="../assets/img/man.svg" alt="">
				</button>
			</div>
		</div>
	</div>
</div>
</template>

<script>
	// import axios from 'axios'
	import Modal from '../components/Modal'
	import Edit from '../components/Edit'
	export default {
		data() {
			return {
				editObj:{}
			}
		},
		
		computed:{
			obj(){
				return this.$store.getters.getObjs
			}
		},
		components: {
			Modal, Edit
		},
		methods: {
			man() {
				this.$store.commit('changeModal', true)
			},
			del(index){
				let patient ={
					index:index,
					id:this.obj[index].id
				}
				this.$store.dispatch('delPatient',patient)
			},
			edit(index){
				this.editObj = this.obj[index]
				this.$store.commit('editModalToggle',true)

			}
		},

	}
</script>

<style>
	.patients {
		width: 100%;
		border-radius: 8px;
		background-color: white;
		box-shadow: 0px 12px 26px rgba(16, 30, 115, 0.06);
		margin-left: 30px;
	}

	.title {
		font-size: 20px;
		font-family: 'lb';
		color: #25282B;
		padding-top: 24px;
		padding-left: 24px;
		margin-bottom: 18px;
	}

	table {
		width: 100%;
		border-collapse: collapse;
		padding: 24px;
	}

	tr {
		height: 56px;
	}

	th {
		font-size: 14px;
		font-family: 'lb';
		color: #25282B;
		padding-top: 18px;
		background-color: #E8E8E8;

	}

	td {
		padding-left: 24px;
		font-size: 14px;
		font-family: 'lr';
		color: #52575C;
	}

	.del {
		margin-right: 28px;
		margin-left: 16px;
		border: 0;
		background-color: white;
	}

	.del:focus {
		outline: 0;
	}

	.edit {
		border: 0;
		background-color: white;
	}

	.edit:focus {
		outline: 0;
	}

	.list {
		height: 56px;
	}

	.name {
		margin-top: 12px;
		align-items: center;
		display: flex;
	}

	.name img {
		width: 32px;
		height: 32px;
		margin-right: 12px;
	}

	.next {
		display: flex;
		margin-top: 20px;
		justify-content: space-between;
	}

	.numbers {
		display: flex;
	}

	.one {
		width: 32px;
		height: 32px;
		background-color: white;
		margin-left: 4px;
		border-radius: 4px;
		border: 1px solid #E8E8E8;
	}

	.man {
		width: 64px;
		height: 64px;
		background-color: #336CFB;
		border-radius: 50%;
		border: 0;
	}

	.man:focus {
		outline: 0;
	}

	.more {
		border: 0;
		background-color: white;
		margin-right: 15px;
	}

	.more:focus {
		outline: 0;
	}
</style>